        ...
        <el-main>
            <el-container style="height: 100%;" :key="locale && locale.name">
                <el-aside class="_fc-l-menu" v-show="!hiddenLeft" width="40px" v-if="false !== getConfig('showMenuBar')">
                    ...
                    <el-tooltip v-if="getConfig('showAi', true)" effect="dark" :content="t('ai.name')" placement="right" :hide-after="0">
                ...
                <el-aside class="_fc-l" v-if="!hiddenLeft" :width="activeModule === 'language' || activeModule === 'ai' ? '450px' : '266px'">
                    ...
                    <el-container  v-if="activeModule === 'base'" direction="vertical">
                    ...
                    <el-container style="height: 100%;">
                        ...
                        <el-main class="_fc-r-tab-form" v-show="activeTab==='form'"
                            ...
                            <DragForm :rule="form.rule" :option="form.option"
                                      ...
                                      v-model="form.api" @mounted="formMounted">
                            ...
                            <template
                                ...
                                <template v-if="activeRule">
                                    <ConfigItem :label="t('props.hide')">
                                        <el-switch size="mini" :value="activeRule._hidden" @input="toolHidden(activeRule)"></el-switch>
                                    </ConfigItem>
                                </template>
                            </template>
    ...
    setup(props, {emit}) {
        ...
        const methods = {
            ...
            toolHidden(rule) {
                ...
                if (rule._menu.inside) {
                    $set(rule.children[0].props, 'hidden', status);
                } else {
                    $set(rule.__fc__.parent.rule.props, 'hidden', status);
                }
                $set(rule, '_hidden', status);
                if (!status) {
                    $set(rule, '_display', true);
                }
            },
